...
const char* scl_vfmt_static(const char* fmt, va_list args) {
...
const char* scl_fmt_static(const char* fmt, ...) {
  const char* msg;
  va_list args;
  va_start(args, fmt);
...
const char* scl_vfmt(const char* fmt, va_list args) {
  va_list copy;
  va_copy(copy, args);
...
const char* scl_fmt(const char* fmt, ...) {
  const char* msg;
  va_list args;
  va_start(args, fmt);
...
scl_file* scl_open(const char* mode, const char* path) {
  if(!mode || !path)
    return NULL;
...
scl_file* scl_openf(const char* mode, const char* path_fmt, ...) {
  va_list args;
  va_start(args, path_fmt);
  const char* path = scl_vfmt(path_fmt, args);
  va_end(args);
...
unsigned scl_read(scl_file* F, void* buffer, const unsigned n) {
  if(!F)
    return 0;
...
unsigned scl_read_malloc(scl_file* F, void** buffer, const unsigned n) {
  if(!buffer)
    return 0;
  unsigned size = scl_fsize(F);
...
int scl_write(scl_file* F, const void* buffer, const int n) {
  if(!F || !F->stream || !buffer || n < 0)
    return -1;
  return fwrite(buffer, 1, n, (FILE*)F->stream);
}

int scl_write_str(scl_file* F, const char* str) {
...
const char* scl_realpath(const char* rel) {
...
const char* scl_parentpath(const char* path) {
  if(!path)
    return NULL;
  const char* abs = scl_realpath(path);
  int l = strlen(abs);
  char* p = (char*)abs + l - 1;
  int n = -1;
  for(; *p && p >= abs; --p)
  ...
  n = p - abs;
  const char* out = p != abs ? scl_strncopy(abs, n) : scl_strcopy(".");
  free((void*)abs);
  return out;
}
...
const char* scl_filename(const char* path) {
  ...
  n = p - path;
  const char* out = p != path ? scl_strncopy(p, n) : scl_strcopy(".");
  free((void*)path);
  return out;
}

int scl_exists(const char* path) {
  const char* abs = scl_realpath(path);
...
int scl_existsf(const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
  const char* path = scl_vfmt(fmt, args);
...
long scl_wtime(const char* path) {
#if defined(__unix__) || defined(__APPLE__)
  stat_t s = {0};
  if(stat(path, &s) == -1) {
    return 0;
...
int scl_mkdir(const char* path) {
#if defined(__unix__) || defined(__APPLE__)
  stat_t s = {0};
  const char* npath = scl_realpath(path);
  if(stat(npath, &s) == -1) {
    mkdir(npath, 0755);
    return 1;
  }
...
static int scl_scandir_(char const* dir, char const* mask, char*** buf_,
  ...
  const char* spec = scl_fmt_static("%s\\%s", dir, mask);
  ...
  do {
    if(!!strcmp(ffd.cFileName, ".") && !!strcmp(ffd.cFileName, "..") &&
      ...
      const char* I = scl_fmt("%s\\%s", dir, ffd.cFileName);
  ...
  while(handle) {
    ...
    if((dp = readdir(handle))) {
      if(!!strcmp(dp->d_name, ".") && !!strcmp(dp->d_name, "..")) {
        ...
        const char* path = scl_fmt("%s/%s", dir, dp->d_name);
...
const char** scl_scandir(const char* dir, const char* mask, int* count) {
  char** buf = NULL;
  char* dsect = NULL;
  int n = 0;
  int m = -1;
...
static int scl_glob_(const char* dir, const char* mask, char*** buf_,
  char** dsect_, int* n_, int* m_) {
  char** buf = *buf_;
  char* dsect = *dsect_;
  int n = *n_;
...
const char** scl_glob(const char* dir, const char* mask, int* count) {
  char** buf = NULL;
  char* dsect = NULL;
  int n = 0;
  int m = -1;
...
int scl_utf8_strlen(const char* str) {
  if(!str)
    return -1;
  int s = 0;
...
int scl_utf8_actual(const char* str, int ind) {
  if(!str)
    return -1;
  int s = 0;
...
int scl_utf8_at(const char* str, int ind) {
  if(!str)
    return -1;
...
int scl_utf8_decode(unsigned int utf8) {
  int out = 0;
  const unsigned char* utf8str = (unsigned char*)&utf8;
...
const char* scl_strncopy(const char* str, int n) {
  n = n >= 0 ? n : 0;
...
const char* scl_strcopy(const char* str) {
  int l = strlen(str ? str : "");
  return scl_strncopy(str, l);
}

int scl_strnffi(const char* str, const char* cs, int n) {
  if(!str || !cs)
    return -1;
  const char* p = str;
  int csl = n;
...
int scl_strffi(const char* str, const char* cs) {
  if(!str || !cs)
    return -1;
  return scl_strnffi(str, cs, strlen(cs));
}

const char* scl_strncat(
  const char* str, const char* str2, int n, int n2, char freestr) {
  if(!str && !str2)
    return NULL;
  n = n >= 0 ? n : 0;
  n2 = n2 >= 0 ? n2 : 0;
...
const char* scl_strcat(const char* str, const char* str2, char freestr) {
  return scl_strncat(
    str, str2, strlen(str ? str : ""), strlen(str2 ? str2 : ""), freestr);
}

const char* scl_strncat2(char* str, const char* str2, int n, int n2) {
  if(!str)
    return NULL;
  n = n >= 0 ? n : 0;
  n2 = n2 >= 0 ? n2 : 0;
...
const char* scl_strcat2(char* str, const char* str2) {
  return scl_strncat2(
    str, str2, str ? strlen(str) : 0, str2 ? strlen(str2) : 0);
}

const char* scl_strreplace(const char* str, const char* old, const char* with) {
  if(!str || !old)
    return NULL;
  char* out = NULL;
  while(1) {
    int p = scl_strffi(str, old);
    const char* n = scl_strncat(
      out, str, strlen(out ? out : ""), p >= 0 ? p : strlen(str), 0);
    if(out)
      free(out);
    if(p < 0) {
      out = (char*)n;
    ...
    if(with) {
      const char* n2 = scl_strcat(n, with, 0);
      free((void*)n);
      out = (char*)n2;
    } else
      out = (char*)n;
...
const char* scl_randstr(int len) {
...
static char match(const char* pattern, const char* candidate, int p, int c) {
  if(pattern[p] == '\0') {
...
char scl_strmatch(const char* str, const char* pattern) {
...
unsigned int scl_strhash(const char* str) {
...
#define hisoptimal(t)   (!hisnstrained(t) && !hisnbig(t) && (t->hsz) >= HTAB_MIN)
#define hoptimal(t) \
...
void scl_htabset(scl_htab* h, const char* key, const void* ptr) {
  if(!hisoptimal(h))
    scl_htaboptimize(h);
...
void scl_htabremove(scl_htab* h, const char* key) {
...
const void* scl_htabget(const scl_htab* h, const char* key) {
  unsigned hash = scl_strhash(key);
  scl_hnode n = gnodehash(h, hash);
...
const void* scl_htabnext(const scl_htab* h, const char* key) {
  if(!h->hnum)
    return NULL;
  if(!key)
...
scl_htab* scl_htabcopy(const scl_htab* h) {
  scl_htab* out = scl_htabnew();
  const char* k = NULL;
  while((k = scl_htabnext(h, k))) {
    const void* ptr = scl_htabget(h, k);
...
static int xml_viewstrncmp(xml_view sv, const char* str, int n) {
  if(!str)
    return 1;
...
static int xml_viewstrcmp(xml_view sv, const char* str) {
  if(!sv.p || !str)
    return 1;
...
static xml_view xml_parse_text(const char* s, const char** ep, char delim) {
  const char* p = s;
  while(*p && *p != delim)
    p++;
...
static xml_attr* xml_parse_attr(xml_doc* doc, const char* s, const char** ep) {
  xml_attr attr;
  const char* p = s;
  if(!xisalnum(*p))
    return NULL;
...
static xml_elem* xml_parse_elem(
  xml_doc* doc, xml_elem* parent, const char* s, const char** ep) {
  static int leave = 0;
  const char* p = s;
  xskipspace(p);
  s = p;
...
xml_doc* xml_load_string(const char* str) {
  xml_doc doc;
  ...
  memcpy(p, str, l);
  p[l] = 0;
  xml_elem* root = xml_parse_elem(&doc, NULL, p, (const char**)&p);
  if(!root) {
    scl_freepages(&doc.txt);
    scl_freepages(&doc.nodes);
    return NULL;
...
xml_doc* xml_load_file(const char* path) {
...
xml_elem* xml_new_elem(xml_doc* doc, const char* tag, const char* str) {
  if(!tag)
    return NULL;
  xml_elem elem;
...
xml_attr* xml_str_attribute(const char* tag, const char* str) {
  if(!tag || !str)
    return NULL;
  xml_attr attr;
  memset(&attr, 0, sizeof(attr));
...
xml_attr* xml_int_attribute(const char* tag, int i) {
  if(!tag)
    return NULL;
  xml_attr attr;
  memset(&attr, 0, sizeof(attr));
...
xml_attr* xml_float_attribute(const char* tag, float n) {
  if(!tag)
    return NULL;
  xml_attr attr;
  memset(&attr, 0, sizeof(attr));
...
xml_attr* xml_find_attribute(xml_elem* elem, const char* tag) {
...
void xml_remove_attribute(xml_elem* elem, const char* tag) {
  if(!elem || !elem->attr)
    return;
...
static int xml_print_string(
  xml_view v, char** out, char** wp, int* size, char keepquot) {
  char* p = (char*)v.p;
  for(; p < v.e; p++) {
    if(*p == '<') {
...
static int xml_print_attr(xml_attr* attr, char** out, char** wp, int* size) {
  ...
  const char* q = !aposdelim ? "\"" : "\'";
  const char* q2 = !aposdelim ? "=\"" : "=\'";
  xml_napp(out, wp, size, " ", 1);
...
static int xml_print_elem(
  xml_elem* elem, char** out, char** wp, int* size, char isroot) {
  if(!elem)
    return 0;
  xml_elem* ielem = elem;
...
const char* xml_print(xml_doc* doc) {
  if(!doc)
    return NULL;

  const char prologue[] = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
  // char const prologue[] = "";
  int size = 512;
...
xpath_exp* xml_xpath(const char* exp) {
    ...
    if(*p == '/') {
      ...
      if(xisalnum(p[1])) {
        ...
        if(xisdigit(*p)) {
          ...
          const char* tmp = scl_strncopy(s, p - s);
          math.n = atof(tmp);
          free((void*)tmp);
