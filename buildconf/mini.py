...
def source_write(fout, doguards: bool = True):
  if doguards:
...
def minigen(path):
  with open(path, "w") as fout:
    fout.write(f"/*  {Path(path).name} auto generated by mini.py  */\n\n/* clang-format off */\n\n")
...
# initial generation
with minigen(out) as fout:
  header_write(fout)
  if out2 == "":
    source_write(fout)

# followup generation for header/source combo
if out2 != "":
  with minigen(out2) as fout:
    fout.write(f'#include "{out}"\n\n')
    source_write(fout, False)
